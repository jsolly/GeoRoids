...
Current direction lives in `style-guide.md`, `palette-ships-factions.md`, `ships-and-factions.md`, and `eo-satellites/roster-and-briefs.md`. The existing repository's ship implementation notes take precedence over the older source pack's contradictory optional-Hook and temporary-saucer language. There are exactly two player kits, Surveyor and Hauler.
...
The six EO SVGs and `eo-satellites/runtime-contact.png` derive from `src/entities/satellite/eoOutlines.ts`. The three mineral SVGs and `personality-roids/runtime-contact.png` derive from the same contour/facet data used by the Canvas renderer.
