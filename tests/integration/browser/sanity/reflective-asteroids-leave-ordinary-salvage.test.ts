...
for (const viewport of [
  ...
  test(
    `reflective shots leave ordinary salvage and normal firing continues on ${viewport.name}`,
    async () => {
      ...
      try {
        const previousLoot = new Set((await game.getLoot()).map((drop) => drop.id));
        ...
        let shard: Awaited<ReturnType<GameInteractions['getLoot']>>[number] | undefined;
        await expect
          .poll(
            async () => {
              shard = (await game.getLoot()).find(
                (drop) =>
                  drop.kind === 'shard' &&
                  !previousLoot.has(drop.id) &&
                  Math.hypot(drop.x - origin.x, drop.y - origin.y) < 80
              );
              return shard;
            },
            { timeout: 5000 }
          ...
          .toBeDefined();
        assert.ok(shard, 'Reflector left ordinary salvage');
        await game.placeShipAt(shard.x, shard.y);
        await expect
          .poll(async () => (await game.getLoot()).some((drop) => drop.id === shard?.id), {
            timeout: 5000,
          })
          .toBe(false);
        expect(await page.locator('#flight-upgrade').count()).toBe(0);
        await game.placeShipAt(FIRING_POSITION.x, FIRING_POSITION.y);
        const previousShots = await page.evaluate(() =>
          Object.keys(window.__reflectionProof?.shots ?? {})
        );
        await fireAt(game, page);
        await expect
          .poll(
            () =>
              page.evaluate(
                (ids) =>
                  Object.entries(window.__reflectionProof?.shots ?? {}).filter(
                    ([id]) => !ids.includes(id)
                  ),
                previousShots
              ),
          ...
          .not.toEqual([]);
        const proof = await page.evaluate(() => window.__reflectionProof?.shots ?? {});
        expect(
          Object.entries(proof)
            .filter(([id]) => !previousShots.includes(id))
            .every(([, shot]) => shot.energy === 1)
        ).toBe(true);
        expect(consoleState.errors).toEqual([]);
        expect(consoleState.warnings).toEqual([]);
        await page.screenshot({
          path: screenshotManager.getScreenshotPath(`reflective-salvage-${viewport.name}.png`),
        });
        writeFileSync(
          screenshotManager.getScreenshotPath(`reflective-salvage-${viewport.name}.json`),
          `${JSON.stringify({ reflectorId: REFLECTOR_ID, shardId: shard.id, shots: proof }, null, 2)}\n`
        );
      } catch (error) {
