...
export const gameReference: Record<string, { heading: string; paragraphs: string[] }[]> = {
  ...
  scout: [
    ...
    },
    {
      ...
      paragraphs: [
        `With Mineral Scan equipped, E runs one ${seconds(SHIP_ABILITY.SCAN_FRAMES)} radar pulse within ${SHIP_ABILITY.SCAN_RANGE} units. That thin cyan sweep expands from the Scout to the viewport edge; this visual cue does not expand the scan range. Spiders inside that range flee and cannot bite while the pulse is active. The next scan waits out the ${seconds(SHIP_ABILITY.COOLDOWN_FRAMES.scout)} cooldown; each identified rock keeps its classification and records the Scout player ID for a later furnace delivery.`,
