...
export async function installAudioProbe(page: Page, enabled = true): Promise<void> {
  await page.addInitScript((soundEnabled) => {
    ...
    AudioParam.prototype.setTargetAtTime = function (this: AudioParam, value, startTime, constant) {
      ...
      queueMicrotask(() => {
        ...
        for (let hop = 0; node && hop < 8; hop++) {
          if (node instanceof PannerNode) {
            ...
            if (state) {
              event.position = { ...state, model: node.panningModel, rolloff: node.rolloffFactor };
            }
            break;
          }
