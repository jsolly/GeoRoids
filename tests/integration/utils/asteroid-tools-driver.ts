...
export function captureConsole(page: import('playwright').Page): { errors: string[]; warnings: string[] } {
  const errors: string[] = [];
...
export async function driveToAsteroid(
  ...
  try {
    while (Date.now() < deadline) {
      ...
      if (!navigation) throw new Error(`Asteroid ${asteroidId} disappeared while piloting`);
      last = navigation;
      if (!navigation.alive) throw new Error(`Pilot died while approaching asteroid ${asteroidId}`);
      if (navigation.gap <= stopGap) break;
...
export function nearestReflectiveCluster(targets: EnhancedTarget[], ship: WorldPoint): EnhancedTarget[] {
  const groups = new Map<string, EnhancedTarget[]>();
  for (const target of targets) {
    if (target.phenomenon?.kind !== 'reflective' || !target.phenomenon.clusterId) continue;
  ...
  return [...groups.values()]
    ...
    .sort((a, b) => {
      const distance = (group: EnhancedTarget[]) =>
        Math.min(...group.map((target) => Math.hypot(target.position.x - ship.x, target.position.y - ship.y)));
