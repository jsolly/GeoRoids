...
`server/world/WorldStore.ts` uses SQLite with WAL and full synchronous transactions. The database stores world seed, start time, generation, UTC score season, completed sectors, shared exploration, visited sector contents, and pilot monthly scores. Ordinary flight and partial mining checkpoint every five seconds; terminal asteroid breaks, deliveries, score-bearing loot collections, successful loot blasts, new pilot credentials, and the final player's departure checkpoint immediately. Graceful shutdown checkpoints before closing the database. A hard crash can lose the uncheckpointed ordinary flight or partial-mining interval (normally about five seconds); a committed break or delivery consumes its world object and records its contributor scores in the same transaction.

A private random bearer token identifies a browser's monthly score. The browser stores it locally; the database stores its SHA-256 digest. A valid token restores that score after a reload, a return to the title screen, game over, or a server restart. Enter Game after leaving still uses a fresh spawn, the nickname and kit from the title screen, three lives, and full health. A brief connection drop while the ship is still alive keeps the live pose, lives, and score. Offline Surveyors receive delivery credit on their next return. Clearing browser storage loses that credential. Names and public pilot IDs do not grant access to saved score. Treat the database and browser credentials as private.

Asteroid impacts remove health, and deaths consume lives. Losing the final life ends that flight. Enter Game starts a new flight with three lives and the same monthly score; shared discoveries and the harvested world remain until the UTC calendar month ends. At month rollover the server zeros scores and resets the shared world (exploration, completed sectors, and deposits). Saved worlds also store a UTC score season (`YYYY-MM`); a missing or mismatched season resets the database the same way a generation mismatch does.
