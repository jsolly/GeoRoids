...
export const gameReference: Record<string, { heading: string; paragraphs: string[] }[]> = {
  ...
  controls: [
    ...
    },
    {
      ...
      paragraphs: [
        `E or the ability button inside any lit furnace’s visible footprint opens Furnace travel. B toggles the same menu. The approach prompt shows Press E to travel, or Tap TRAVEL to open map on touch screens. Select another lit furnace on the destination map for a free rocket ride along the pipe network, lasting 0.6–2 seconds. Boarding releases towed cargo and couplings; the ship is protected and cannot act during the ride. The menu holds the ship like the map and schematic. Town Square also sells extra lives within ${TOWN_STORE_RADIUS} units of the origin. Each street a pilot built adds ${Math.round(TOWN_YIELD_PER_MODULE * 100)}% to that pilot's own furnace deliveries. An extra life costs ${EXTRA_LIFE_COST.toLocaleString('en-US')} score, up to ${MAX_LIVES} lives.`,
      ],
    },
  ],
  scout: [
  ...
  teamwork: [
    {
      ...
      paragraphs: [
        ...
        `${TOWN_HEARTH.name} (${TOWN_HEARTH.radius}-unit intake) is the only pre-lit hearth. ${streetLots(1).length} street lots sit in the first band, then ${streetLots(2).length} and ${streetLots(3).length} farther lots scattered through outer bands. A Scout builds the next dark foundation with their own score once its inward parent lot is burning and that score covers ${streetCost(1)}, ${streetCost(2)}, or ${streetCost(3)}. The furnace keeps the builder's name. Every Hauler and recorded Scout receives the size-scaled material reward, and each street that recipient built adds ${Math.round(TOWN_YIELD_PER_MODULE * 100)}% to their own payout. Size-25 base values are ice ${furnaceReward({ material: 'ice', size: 25 })}, metal ${furnaceReward({ material: 'metal', size: 25 })}, and rubble ${furnaceReward({ material: 'rubble', size: 25 })}. The Town Square store, within ${TOWN_STORE_RADIUS} units of the origin, sells one extra life for ${EXTRA_LIFE_COST.toLocaleString('en-US')} score, up to ${MAX_LIVES} lives.`,
      ],
    },
  ],
};
