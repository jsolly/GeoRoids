...
| 2. Behavioral CI | Required `CI / ci` aggregates parallel static checks and serialized gameplay/touch/reconnect smoke. Full unit/integration/performance checks are mandatory locally; V8 coverage is manual; weekly/manual full integration remains a backstop. | See [CI and local review](../ci-and-local-review.md) for the current gate and timing evidence. No coverage percentage threshold is claimed. |
