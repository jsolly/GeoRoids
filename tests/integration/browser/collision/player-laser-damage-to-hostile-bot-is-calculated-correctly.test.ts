...
test('a player laser deals canonical damage to a hostile bot', async () => {
  ...
  await expect
    .poll(
      () => botUpdates.slice(updateCountBeforeShot).find(
        (update) => update.botId === bot.id && update.health !== undefined && update.health < initialHealth
      ),
      { timeout: 5000, message: 'server should broadcast the accepted hostile bot laser hit' }
    )
    .toBeDefined();
  const damageUpdate = botUpdates.slice(updateCountBeforeShot).find(
    (update) => update.botId === bot.id && update.health !== undefined && update.health < initialHealth
  );
  expect(damageUpdate?.health, 'server should report the bot health after this hit')
    .toBeCloseTo(initialHealth - 25, 0);
  expect(result.minHealthObserved, 'the live bot view should show the health reduction')
