...
The local radar spans 3,600 units. Its 125-unit survey cells are permanent shared discoveries. Town Square is the only hearth that starts lit. Dark street foundations stay on the universe map, and on the local radar while they are inside it. Lit hearths appear on the local radar after the crew reveals them. Built streets keep the Scout's name and the public id of the pilot who paid. They survive death, reconnects, and server restarts; changing `WORLD.generation` clears them on the next start. Scouts pay for a street with their own score. Each street that pilot built raises only their later furnace deliveries by a fixed percent. Other pilots keep the base reward. Unnamed streets saved before that id existed grant no bonus. The Town Square store, inside a fixed radius of the origin, sells one extra life for personal score, up to the life cap. Scouts passively explore farther and can scan a larger area. Scanned cargo records its Scout contributors until consumption.
