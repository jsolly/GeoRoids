...
if ! dest="$(git rev-parse --show-toplevel)"; then
  echo "ERROR worktree-init: could not find the Git root; skipping optional provisioning" >&2
  exit 0
fi
...
if ! primary="$(git worktree list --porcelain 2>/dev/null | sed -n 's/^worktree //p' | head -n1)"; then
  echo "ERROR worktree-init: could not inspect Git worktrees; skipping optional provisioning" >&2
  exit 0
fi
[ -n "$primary" ] || exit 0
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  case "/$line/" in
    //*|*/../*) echo "ERROR worktree-init: skipping unsafe .worktreeinclude entry '$line'" >&2; continue ;;
  esac
  for src in "$primary"/$line; do                     # glob-expand against the primary
    ...
    if ! mkdir -p "$dest/$(dirname "$rel")"; then
      echo "ERROR worktree-init: could not create destination for '$rel'; continuing without optional copy" >&2
      continue
    fi
    if cp -p "$src" "$dest/$rel"; then
      echo "worktree-init: copied $rel" >&2
    else
      echo "ERROR worktree-init: failed to copy '$rel'; continuing without optional copy" >&2
    fi
  done
done < "$manifest"
