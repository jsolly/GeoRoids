...
test('a medium ice roid breaks without producing fragments', async () => {
  ...
  const medium = (await game.getAsteroidPositions()).find(
    (asteroid) => asteroid.radius >= 25 && asteroid.radius < 40 &&
      !asteroid.isCollabTarget && asteroid.material === 'ice'
  );
  expect(medium, 'expected a natural medium ice asteroid').toBeDefined();
  if (!medium) throw new Error('Medium ice asteroid missing');
  received.length = 0;
  await game.destroyAsteroidWithLaser(medium);
  await expect.poll(() => received.find((message) =>
    message.type === 'asteroidDestroy' && message.data?.asteroidId === medium.id
  )).toBeDefined();
  const index = received.findIndex((message) =>
    message.type === 'asteroidDestroy' && message.data?.asteroidId === medium.id
  );
  ...
  await expect.poll(() => received.slice(index + 1).some((message) =>
  ...
  const nearbyFragments = received.flatMap((message) =>
    message.type === 'asteroidCreateBatch' ? message.data.asteroids : []
  ).filter((asteroid: any) => asteroid.material === 'ice' && asteroid.size < medium.radius &&
    Math.hypot(asteroid.position.x - origin.x, asteroid.position.y - origin.y) <= medium.radius);
  expect(nearbyFragments, 'this ice break must not create smaller rocks at its impact').toEqual([]);
  expect((await game.getAsteroidPositions()).some((asteroid) => asteroid.id === medium.id)).toBe(false);
