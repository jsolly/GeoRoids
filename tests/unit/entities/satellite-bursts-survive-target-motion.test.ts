...
describe('EO firing patterns remain recognizable while a pilot moves', () => {
  ...
  test('GOES holds a rapid collinear beam and ENVISAT sweeps the opposite direction', () => {
    ...
    const firstRadarShot = radar[0];
    const secondRadarShot = radar[1];
    assert.ok(firstRadarShot);
    assert.ok(secondRadarShot);
    expect(secondRadarShot.frame - firstRadarShot.frame).toBe(6);
    expect(angleDifference(secondRadarShot.angle, firstRadarShot.angle)).toBeCloseTo(-0.24);
  });
  ...
  test('WorldView fires a faster single precision shot than Landsat', () => {
    ...
    assert.ok(worldviewShot);
    assert.ok(landsatShot);
...
test('arena resets and independent worlds cannot reuse satellite or pickup identities', () => {
  ...
  assert.ok(oldSatellite);
  manager.clearSatellites();
  const nextSatellite = manager.createSatellites(1)[0];
  assert.ok(nextSatellite);
  expect(nextSatellite.id).not.toBe(oldSatellite.id);
  expect(manager.damageSatellite(oldSatellite.id, 100)).toBeUndefined();
  const separateSatellite = new SatelliteManager(new RNGService(42)).createSatellites(1)[0];
  assert.ok(separateSatellite);
  expect(separateSatellite.id).not.toBe(nextSatellite.id);
