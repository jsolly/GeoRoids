...
describe('asteroid collision-fixture placement', () => {
  test('opt-in placement puts each rock exactly on a named player position', () => {
    ...
    const asteroids = field().createAsteroids(5, { radius: 3100 }, playerPositions);
  ...
  test('normal placement is off by default and player hints do not pin the field', () => {
    expect(DEBUG.ROIDS.PLACE_ON_LOCAL_PLAYER).toBe(false);
    const baseline = field().createAsteroids(4, { radius: 3100 }, []);
    const withParticipants = field().createAsteroids(4, { radius: 3100 }, playerPositions);
    ...
    for (const asteroid of withParticipants) {
      expect([...playerPositions]).not.toContainEqual(asteroid.position);
    }
  });
});
