...
The playable roster is Scout and Hauler. Scout uses the delta-wing
...
The current pack is `georoids-art/ships-v2/{scout,hauler}.svg`.
Stroke is `#5EEAD4` on `#000011`. Scout play-scale is `SHIP.SIZE` (30px);
...
| Kit | Hull | Role |
| --- | --- | --- |
| Scout | Delta-wing scout with a forward dish | Nimble exploration and radar mineral scan |
...
- Deliver to a revealed furnace for shared Hauler and Scout rewards
- Tether VFX is Hauler only — cream line `#E8D5A3` + amber tip `#FDE68A`
- Never draw the cream harpoon cable on Scout
