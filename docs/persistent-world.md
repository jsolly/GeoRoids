...
The local radar spans 3,600 units. Its 125-unit survey cells are permanent shared discoveries. Town Square is the only hearth. It appears on the local radar after the crew reveals it, and it stays marked on the universe map. Changing `WORLD.generation` clears saved progress on the next start. The Town Square store, inside a fixed radius of the origin, sells one extra life for personal score, up to the life cap. Surveyors passively explore farther and can scan a larger area. Scanned cargo records its Surveyor contributors until consumption.
...
Asteroid impacts remove health, and deaths consume lives. Losing the final life ends that flight and resets the score to 0. Enter Game then starts a new flight with three lives; shared discoveries and harvested deposits remain. A UTC month boundary does not clear them. Changing `WORLD.generation` resets sectors, pilots, and scores on the next start.
