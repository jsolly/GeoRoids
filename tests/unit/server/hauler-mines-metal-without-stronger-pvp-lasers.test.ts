...
test('Hauler breaks metal in two shots while Surveyor needs three and PvP damage stays normal', () => {
  ...
  expect(surveyor.health).toBe(health - DAMAGE.LASER_HIT);
});
...
test('Hauler improves collaborative HP mining while ordinary one-hit rocks stay one-hit', () => {
  engine.addPlayer('miner', 'Miner', new RecordingSocket(), { x: 0, y: 0 }, undefined, 'hauler');
