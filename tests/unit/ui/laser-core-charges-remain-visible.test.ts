...
test('core pickup reports charges and disappears on expiry, depletion or session reset', () => {
  ...
  expect(document.querySelector('#flight-upgrade')?.textContent).toContain('6 charges');
  readout.update({ ...upgrade, charges: 2 });
  expect(document.querySelector('#flight-upgrade')?.textContent).toContain('2 charges');
  for (const empty of [{ ...upgrade, expiresAt: 0 }, { ...upgrade, charges: 0 }, undefined]) {
    readout.update(empty);
    expect(document.querySelector<HTMLElement>('#flight-upgrade')?.hidden).toBe(true);
  }
  expect(
