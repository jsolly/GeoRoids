...
export const gameReference: Record<string, { heading: string; paragraphs: string[] }[]> = {
  ...
  scout: [
    ...
    },
    {
      ...
      paragraphs: [
        `With Mineral Scan equipped, E runs one radar pulse lasting ${seconds(SHIP_ABILITY.SCAN_FRAMES)} within ${SHIP_ABILITY.SCAN_RANGE} units. That thin cyan sweep expands from the Scout to the viewport edge; this visual cue does not expand the scan range. Spiders inside that range flee and cannot bite while the pulse is active. The scan cooldown is ${seconds(SHIP_ABILITY.COOLDOWN_FRAMES.scout)}; each identified rock keeps its classification and records the Scout player ID for a later furnace delivery.`,
